...
void initialize_model(KeyCopierModel* model) {
    ...
    model->inches_per_px = INCHES_PER_PX;
    model->x_offset_px = 10;
}
...
static void key_copier_view_measure_draw_callback(Canvas* canvas, void* model) {
    ...
    double inches_per_px = my_model->inches_per_px;
    int x_offset = my_model->x_offset_px;
    ...
    int level_contour_px =
        x_offset + (int)round((my_format.last_pin_inch + my_format.elbow_inch) / inches_per_px);
    for(int current_pin = 1; current_pin <= my_model->format.pin_num; current_pin += 1) {
        ...
        int pin_center_px = x_offset + (int)round(current_center_px / inches_per_px);
        ...
        if(current_pin == 1) {
            ...
            if(my_format.sides == 2) {
                ...
                canvas_draw_line(canvas, x_offset, 62, level_contour_px, 62);
            }
        }
    ...
    canvas_draw_line(canvas, x_offset, top_contour_px - 6, x_offset, top_contour_px);
    if(my_format.stop == 2) {
    ...
    int slc_pin_px = x_offset + (int)round(
    ...
    if(event->type == InputTypeShort) {
        ...
        if(key_char == '[') {
            ...
            with_view_model(
                ...
                KeyCopierModel * model,
                {
                    if(model->inches_per_px > 0.003) {
                        model->inches_per_px -= 0.0001;
                    }
                },
                redraw);
