...
void furi_hal_display_init(void) {
    ...
    spi_bus_config_t bus_cfg = {
        ...
        /* On shared-bus boards the LCD and SD card sit on the same SPI host, so
         * the bus must carry the SD MISO line (the LCD itself is write-only).
         * Boards that put the SD card on a SEPARATE host (e.g. Cardputer: LCD on
         * SPI2, SD on SPI3) define BOARD_PIN_LCD_SPI_MISO = -1 so we do NOT claim
         * the SD MISO pin here — otherwise that GPIO is owned by two SPI buses at
         * once and bus init fails / the panel never drives. */
