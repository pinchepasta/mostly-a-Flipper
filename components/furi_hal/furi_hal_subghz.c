...
static void furi_hal_subghz_async_tx_middleware_idle(FuriHalSubGhzAsyncTxMiddleware* middleware) {
    ...
    ESP_LOGD(TAG, "Middleware reset to idle state");
}

static uint32_t IRAM_ATTR furi_hal_subghz_async_tx_middleware_get_duration(FuriHalSubGhzAsyncTx* async_tx) {
    ...
    while(iterations < max_iterations) {
        ...
        if(level_duration_is_reset(ld)) {
            ...
            uint32_t result = middleware->is_odd_level ? middleware->adder_duration : 0;
            return result;
        } else if(level_duration_is_wait(ld)) {
            // When data is not available, return guard time to pause transmission
            // This prevents calling the callback repeatedly in ISR context
    ...
    // If max iterations reached, return to prevent system hang
    middleware->state = FuriHalSubGhzAsyncTxMiddlewareStateReset;
    return 0;
}
...
static bool IRAM_ATTR furi_hal_subghz_async_tx_next_chunk(
    FuriHalSubGhzAsyncTx* async_tx,
    bool* level,
    uint16_t* duration) {
    furi_check(level);
...
static size_t IRAM_ATTR furi_hal_subghz_async_tx_encoder_callback(
    ...
    while(encoded < symbols_free) {
        ...
        if(!furi_hal_subghz_async_tx_next_chunk(async_tx, &level1, &duration1)) {
            // When second chunk fails, end transmission
            level1 = false;
            duration1 = 1;
            *done = true;
        }

        // Validate durations to prevent RMT driver issues
    ...
    if(encoded == 0 && symbols_free > 0) {
        // If no symbols encoded but space available, output a guard symbol
        symbols[0].level0 = 0;
        symbols[0].duration0 = FURI_HAL_SUBGHZ_RMT_MAX_DURATION_TICKS;
        symbols[0].level1 = 0;
        symbols[0].duration1 = 1;
        encoded = 1;
        // Don't set done, let encoder be called again
    }
    return encoded;
}
...
static bool IRAM_ATTR furi_hal_subghz_async_tx_done_callback(
    ...
    if(!furi_hal_subghz.async_tx_complete) {
...
static void furi_hal_subghz_async_tx_free_backend(FuriHalSubGhzAsyncTx* async_tx) {
    ESP_LOGI(TAG, "Freeing RMT backend");
    
    if(async_tx->channel) {
        ESP_LOGD(TAG, "Disabling RMT channel");
    ...
    // Small delay to allow RMT hardware to settle before deletion
    furi_delay_ms(2);

    if(async_tx->encoder) {
        ESP_LOGD(TAG, "Deleting RMT encoder");
    ...
    ESP_LOGI(TAG, "RMT backend freed successfully");
}
...
bool furi_hal_subghz_start_async_tx(FuriHalSubGhzAsyncTxCallback callback, void* context) {
    ...
    ESP_LOGI(TAG, "start_async_tx: regulation=%d", furi_hal_subghz.regulation);
    
    if(furi_hal_subghz.regulation != FuriHalSubGhzRegulationTxRx) {
        ESP_LOGE(TAG, "TX not allowed: regulation=%d", furi_hal_subghz.regulation);
        return false;
    }
    ...
    ESP_LOGI(TAG, "RMT backend allocated");

    if(!furi_hal_subghz_tx()) {
        ESP_LOGE(TAG, "furi_hal_subghz_tx failed");
        async_tx->callback = NULL;
        async_tx->callback_context = NULL;
        furi_hal_subghz.async_tx_complete = true;
        furi_hal_subghz.state = FuriHalSubGhzStateIdle;
    ...
    ESP_LOGI(TAG, "CC1101 TX mode set");
    ...
    ESP_LOGI(TAG, "RMT encoder reset");

    static IRAM_ATTR const uint8_t tx_marker = 0xA5;
    ...
    ESP_LOGI(TAG, "Calling rmt_transmit");
    error = rmt_transmit(async_tx->channel, async_tx->encoder, &tx_marker, sizeof(tx_marker), &tx_config);
    if(error != ESP_OK) {
        ESP_LOGE(TAG, "rmt_transmit failed: %s", esp_err_to_name(error));
        furi_hal_subghz_idle();
        furi_hal_subghz_async_tx_free_backend(async_tx);
        async_tx->callback = NULL;
        async_tx->callback_context = NULL;
    ...
    ESP_LOGI(TAG, "RMT transmission started");
    return true;
}
...
void furi_hal_subghz_stop_async_tx(void) {
    if(furi_hal_subghz.state != FuriHalSubGhzStateAsyncTx) {
        ESP_LOGD(TAG, "stop_async_tx called but not in TX state");
        return;
    }

    ESP_LOGI(TAG, "Stopping async TX");
    FuriHalSubGhzAsyncTx* async_tx = &furi_hal_subghz.async_tx;
    async_tx->stop_requested = true;

    if(async_tx->channel && !furi_hal_subghz.async_tx_complete) {
        ESP_LOGD(TAG, "Disabling incomplete transmission");
        esp_err_t error = rmt_disable(async_tx->channel);
        if(error != ESP_OK && error != ESP_ERR_INVALID_STATE) {
    ...
    async_tx->stop_requested = false;
    furi_hal_subghz.state = FuriHalSubGhzStateIdle;
    ESP_LOGI(TAG, "Async TX stopped successfully");
