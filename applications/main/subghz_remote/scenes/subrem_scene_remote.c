...
static uint8_t subrem_scene_remote_event_to_index(SubRemCustomEvent event_id) {
    ...
    } else if(event_id == SubRemCustomEventViewRemoteStartEXTRA) {
        ret = SubRemSubKeyNameExtra;
    } else if(event_id == SubRemCustomEventViewRemoteStart7) {
        ret = SubRemSubKeyName7;
    } else if(event_id == SubRemCustomEventViewRemoteStart8) {
        ret = SubRemSubKeyName8;
    } else if(event_id == SubRemCustomEventViewRemoteStart9) {
        ret = SubRemSubKeyName9;
    }
...
bool subrem_scene_remote_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == SubRemCustomEventViewRemoteBack) {
            ...
            event.event == SubRemCustomEventViewRemoteStartEXTRA ||
            event.event == SubRemCustomEventViewRemoteStart7 ||
            event.event == SubRemCustomEventViewRemoteStart8 ||
            event.event == SubRemCustomEventViewRemoteStart9) {
            // Start sending sub
