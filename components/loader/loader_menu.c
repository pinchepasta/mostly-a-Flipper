...
static void loader_menu_internal_external_apps_callback(void* context, uint32_t index) {
    UNUSED(context);
    const char* launch = FLIPPER_INTERNAL_EXTERNAL_APPS[index].path;
    loader_menu_start(launch);
}

static void loader_menu_blespv3_callback(void* context, uint32_t index) {
    UNUSED(index);
    UNUSED(context);
    DialogsApp* dialogs = furi_record_open(RECORD_DIALOGS);
    if(dialogs) {
        DialogMessage* message = dialog_message_alloc();
        dialog_message_set_header(message, "BLESPv3", 64, 0, AlignCenter, AlignTop);
        dialog_message_set_text(
            message, "Press connect to continue.", 64, 32, AlignCenter, AlignCenter);
        dialog_message_set_buttons(message, "Back", NULL, "Connect");
...
static bool loader_menu_should_hide_main_entry(const char* name) {
    if(!name) {
        return false;
    }
    return strcmp(name, "Tamagotchi") == 0 || strcmp(name, "QRCode") == 0 ||
           strcmp(name, "nRF24 Jammer") == 0 || strcmp(name, "Slots") == 0 ||
           strcmp(name, "Pong") == 0 || strcmp(name, "XRemote") == 0 ||
           strcmp(name, "Key Copier") == 0 || strcmp(name, "Tetris") == 0 ||
           strcmp(name, "ZeroFIDO") == 0 || strcmp(name, "Snake 2.0") == 0 ||
           strcmp(name, "Blackjack") == 0 || strcmp(name, "Blackjack Split") == 0 ||
           strcmp(name, "Game 15") == 0 || strcmp(name, "Asteroids") == 0 ||
           strcmp(name, "Texas Holdem") == 0 || strcmp(name, "DOOM") == 0 ||
           strcmp(name, "Authenticator") == 0 || strcmp(name, "[GPIO] RGB LED") == 0 ||
           strcmp(name, "Dice D&D") == 0;
}
...
static void loader_menu_build_menu(LoaderMenuApp* app, LoaderMenu* menu) {
    ...
    menu_add_item(
        ...
        menu_index++,
        loader_menu_blespv3_callback,
        (void*)menu);

    for(i = 0; i < FLIPPER_APPS_COUNT; i++) {
        const char* name = FLIPPER_APPS[i].name;
        if(loader_menu_should_hide_main_entry(name)) {
            continue;
        }
        menu_add_item(
            app->primary_menu,
    ...
    for(i = 0; i < FLIPPER_EXTERNAL_APPS_COUNT; i++) {
        const char* name = FLIPPER_EXTERNAL_APPS[i].name;
        if(loader_menu_should_hide_main_entry(name)) {
            continue;
        }
        menu_add_item(
    ...
    for(i = 0; i < FLIPPER_INTERNAL_EXTERNAL_APPS_COUNT; i++) {
        const char* name = FLIPPER_INTERNAL_EXTERNAL_APPS[i].name;
        if(loader_menu_should_hide_main_entry(name)) {
            continue;
        }
        menu_add_item(
            app->primary_menu,
            name,
            FLIPPER_INTERNAL_EXTERNAL_APPS[i].icon,
            i,
            loader_menu_internal_external_apps_callback,
            (void*)menu);
        menu_index++;
    }
    ...
    if((FLIPPER_EXTSETTINGS_APPS_COUNT > 0) || (FLIPPER_SETTINGS_APPS_COUNT > 0)) {
        menu_add_item(
            app->primary_menu, "Settings", &A_Settings_14, menu_index++, loader_menu_switch_to_settings, app);
    }
}
