...
void furi_hal_power_init(void) {
     ...
     * A board with no I2C fuel gauge (e.g. Cardputer-ADV uses ADC battery
     * sensing) can set BOARD_POWER_SKIP_I2C to keep this from seizing
     * I2C_NUM_0 — which on those boards belongs to another device such as the
     * keyboard controller. */
#if defined(BOARD_PIN_QWIIC_SDA) && defined(BOARD_PIN_QWIIC_SCL) && \
    !defined(BOARD_POWER_SKIP_I2C)
    {
        i2c_config_t conf = {
