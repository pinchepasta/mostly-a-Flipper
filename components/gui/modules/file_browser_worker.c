...
static bool browser_filter_by_name(BrowserWorker* browser, FuriString* name, bool is_folder) {
    ...
    // Hide specific app entries from the Apps browser listing only.
    if((furi_string_cmp_str(browser->path_start, EXT_PATH("apps")) == 0) &&
       (furi_string_cmp_str(name, "mostlygotchi") == 0)) {
        return false;
    }
