...
static int32_t infrared_app_run(
    InfraredApp* infrared,
    bool is_rpc_mode,
    bool is_remote_loaded,
    bool start_remote_list);
...
static int32_t infrared_app_run(
    ...
    if(is_rpc_mode) {
        ...
        if(is_remote_loaded) { //-V547
            ...
            scene_manager_next_scene(infrared->scene_manager, InfraredSceneStart);
            if(start_remote_list) {
                scene_manager_next_scene(infrared->scene_manager, InfraredSceneRemoteList);
            }
        }
    }
...
int32_t ir_shortcuts_app(void* p) {
    ...
    return infrared_app_run(infrared, is_rpc_mode, is_remote_loaded, true);
}
...
int32_t infrared_app(void* p) {
    ...
    return infrared_app_run(infrared, is_rpc_mode, is_remote_loaded, false);
}
