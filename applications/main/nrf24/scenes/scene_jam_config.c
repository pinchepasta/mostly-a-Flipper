...
static void jam_config_set_spectrum(VariableItem* item) {
    Nrf24App* app = variable_item_get_context(item);
    uint8_t idx = variable_item_get_current_value_index(item);
    nrf24_jam_config_get(app->jam.source, app->jam.protocol)->wide_spectrum = idx ? 1 : 0;
    variable_item_set_current_value_text(item, idx ? "Wide" : "Narrow");
    nrf24_jam_config_touch();
}
...
void nrf24_app_scene_jam_config_on_enter(void* context) {
    ...
    item = variable_item_list_add(vil, "Spectrum", 2, jam_config_set_spectrum, app);
    variable_item_set_current_value_index(item, cfg->wide_spectrum ? 1 : 0);
    variable_item_set_current_value_text(item, cfg->wide_spectrum ? "Wide" : "Narrow");
