...
#define WLAN_BEACON_SSID_FILE_PATH "/ext/wifi/ssid_spam.txt"
#define WLAN_BEACON_MAX_FILE_SSIDS 32
...
static char s_beacon_file_ssids[WLAN_BEACON_MAX_FILE_SSIDS][33];
static uint8_t s_beacon_file_ssid_count = 0;
static uint8_t s_beacon_file_ssid_index = 0;
static char s_beacon_file_path[256] = {0};

static void beacon_trim_line(char* line) {
    char* start = line;
...
static void beacon_load_ssids_from_file(const char* file_path) {
    ...
    s_beacon_file_ssid_count = 0;
    s_beacon_file_ssid_index = 0;

    const char* path = file_path && file_path[0] ? file_path : WLAN_BEACON_SSID_FILE_PATH;
    ...
    while(storage_file_read(file, &c, 1) == 1) {
        if(c == '\n' || c == '\r') {
            if(line_len > 0) {
                line[line_len] = '\0';
                beacon_trim_line(line);
                if(line[0] != '\0' && line[0] != '#' && line[0] != ';' &&
                   s_beacon_file_ssid_count < WLAN_BEACON_MAX_FILE_SSIDS) {
                    strncpy(s_beacon_file_ssids[s_beacon_file_ssid_count], line, 32);
                    s_beacon_file_ssids[s_beacon_file_ssid_count][32] = '\0';
                    s_beacon_file_ssid_count++;
                }
                line_len = 0;
                line[0] = '\0';
            }
...
static void beacon_spam_task(void* param) {
    ...
    while(s_beacon_active) {
        ...
        case WlanHalBeaconModeCustom:
            ...
            current_ssid = gen_ssid;
            break;
        case WlanHalBeaconModeFileList:
            if(s_beacon_file_ssid_count == 0) {
                beacon_load_ssids_from_file(s_beacon_file_path);
            }
            if(s_beacon_file_ssid_count > 0) {
                current_ssid = s_beacon_file_ssids[s_beacon_file_ssid_index++];
                if(s_beacon_file_ssid_index >= s_beacon_file_ssid_count) {
                    s_beacon_file_ssid_index = 0;
                }
            }
            break;
...
void wlan_hal_beacon_spam_start(WlanHalBeaconMode mode, const char* base_ssid, const char* file_path) {
    ...
    s_beacon_file_ssid_count = 0;
    s_beacon_file_ssid_index = 0;
    memset(s_beacon_file_path, 0, sizeof(s_beacon_file_path));
    if(file_path && file_path[0]) {
        strncpy(s_beacon_file_path, file_path, sizeof(s_beacon_file_path) - 1);
    }
