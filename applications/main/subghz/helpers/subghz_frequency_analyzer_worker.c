...
static int32_t subghz_frequency_analyzer_worker_thread(void* context) {
    ...
    while(instance->worker_running) {
        ...
        bool use_target_frequency = instance->tuned_frequency > 0;

        // First stage: coarse scan
        if(use_target_frequency) {
            uint32_t current_frequency = instance->tuned_frequency;
        ...
        float baseline_rssi = RSSI_MIN;
        if(rssi_avg_samples > 0) {
            baseline_rssi = rssi_avg / rssi_avg_samples;
            if(baseline_rssi < RSSI_MIN) {
                baseline_rssi = RSSI_MIN;
            }
        }
        ...
        if(frequency_rssi.rssi_coarse > instance->trigger_level) {
            ...
            uint32_t scan_center = frequency_rssi.frequency_coarse;
            if(use_target_frequency) {
                scan_center = instance->tuned_frequency;
            }
            //for example -0.3 ... 433.92 ... +0.3 step 20KHz
            for(uint32_t i = scan_center - 300000;
                i < scan_center + 300000;
                i += 20000) {
        ...
        if(frequency_rssi.rssi_fine > instance->trigger_level) {
            ...
            instance->filVal = 0;
            if(instance->pair_callback) {
                instance->pair_callback(instance->context, 0, baseline_rssi, false);
            }
        }
    }
...
void subghz_frequency_analyzer_worker_set_frequency(
    SubGhzFrequencyAnalyzerWorker* instance,
    uint32_t frequency) {
    instance->tuned_frequency = frequency;
}
