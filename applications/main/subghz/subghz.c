...
SubGhz* subghz_alloc(bool alloc_for_tx_only) {
    ...
    if(!alloc_for_tx_only) {
        ...
        subghz->subghz_waterfall = subghz_waterfall_alloc(subghz);
        view_dispatcher_add_view(
            subghz->view_dispatcher,
            SubGhzViewIdWaterfall,
            subghz_waterfall_get_view(subghz->subghz_waterfall));
    }
    // Read RAW
...
void subghz_free(SubGhz* subghz, bool alloc_for_tx_only) {
    ...
    if(!alloc_for_tx_only) {
        ...
        view_dispatcher_remove_view(subghz->view_dispatcher, SubGhzViewIdWaterfall);
        subghz_waterfall_free(subghz->subghz_waterfall);
    }
    if(!alloc_for_tx_only) {
        // SubBrute Bruteforcer
