...
#include "boards/board.h" /* must precede the BOARD_SD_SPI_HOST check below:
                           * without it the board macro is invisible and the SD
                           * falls back to SPI2_HOST (the LCD's bus) and is then
                           * skipped by the pin-conflict check on the ADV. */
#include "../fatfs.h"
 ...
 * - T-Embed / Waveshare: SD shares SPI2_HOST with the LCD (legacy path); the
 *   LCD init already called spi_bus_initialize(), so we only attach a device.
 * - M5Stack Cardputer: SD is on its own SPI3 host (board header sets
 *   BOARD_SD_SPI_HOST). Nothing initialises that bus, so we must do it here. */
#if defined(BOARD_SD_SPI_HOST)
#  define SD_SPI_HOST    BOARD_SD_SPI_HOST
#  define SD_SEPARATE_BUS 1
#else
...
static bool sd_prepare_card(void) {
    ...
    ESP_LOGI(TAG, "Initializing SD card on host=%d, CS=GPIO%d", SD_SPI_HOST, gpio_sdcard_cs.pin);

#if SD_SEPARATE_BUS
    /* Boards where the SD card is on a dedicated SPI host (not shared with the
     * LCD) must initialise that bus themselves — the display init only touches
     * BOARD_LCD_SPI_HOST, so this host is otherwise uninitialised.
     *
     * IMPORTANT: drive CS HIGH (deasserted) before touching the bus. If CS
     * floats low during spi_bus_initialize the card enters a stray SPI cycle
     * and its state machine wedges — it then never answers CMD0. */
    {
