...
void nrf24_hw_jammer_start_ex(uint8_t channel, uint8_t pa_level) {
    ...
    nrf24_hw_write_reg(NRF_REG_EN_RXADDR, 0x00);
...
static void nrf24_flood_fill_random(uint8_t* buf) {
    static uint32_t xor_seed = 0xA5A5A55Au;
    for(uint8_t i = 0; i < 32; i += 4) {
        xor_seed ^= (uint32_t)(xor_seed << 13);
        xor_seed ^= (uint32_t)(xor_seed >> 7);
        xor_seed ^= (uint32_t)(xor_seed << 17);
        uint32_t r = xor_seed;
        buf[i + 0] = (uint8_t)(r);
        buf[i + 1] = (uint8_t)(r >> 8);
...
void nrf24_hw_flood_burst(uint8_t channel, uint8_t burst_count, bool turbo) {
    ...
    /* Turbo cycles four spectral patterns (burst_count packets each); plain
    ...
    const uint8_t phase_count = turbo ? 4 : 1;
    furi_hal_gpio_write(&nrf24_ce, true);
    for(uint8_t p = 0; p < phase_count; p++) {
        uint8_t sent = 0;
        uint16_t guard = 0;
        while(sent < burst_count && guard++ < 8000) {
