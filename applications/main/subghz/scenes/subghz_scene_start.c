...
bool subghz_scene_start_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeBack) {
        ...
        if(event.event == SubmenuIndexReadRAW) {
            ...
            return true;
        } else if(event.event == SubmenuIndexWaterfall) {
            scene_manager_set_scene_state(
                subghz->scene_manager, SubGhzSceneStart, SubmenuIndexWaterfall);
            scene_manager_next_scene(subghz->scene_manager, SubGhzSceneWaterfall);
            return true;
        } else if(event.event == SubmenuIndexRFShortcuts) {
            Loader* loader = furi_record_open(RECORD_LOADER);
            loader_start_with_gui_error(loader, "subghz_remote", NULL);
