...
void app_main(void) {
    ...
    /* TEMP DEBUG: log free heap at the desktop so we can confirm the BT-deferral
     * memory fix (expect ~100 KB free with BT deferred, vs ~36 KB before). */
    furi_delay_ms(3000);
    ESP_LOGW("HEAP", "desktop free=%u largest=%u",
