...
bool subrem_scene_start_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == SubmenuIndexSubRemOpenMapFile) {
            ...
            consumed = true;
        } else if(event.event == SubmenuIndexSubRemDeleteMapFile) {
            scene_manager_set_scene_state(
                app->scene_manager, SubRemSceneStart, SubmenuIndexSubRemDeleteMapFile);
            scene_manager_next_scene(app->scene_manager, SubRemSceneDeleteMapFile);
            consumed = true;
