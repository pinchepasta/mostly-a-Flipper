...
#if defined(BOARD_SUBGHZ_SPI_HOST)
    /* Cardputer-ADV: CC1101 lives on its own hardware host (SPI3), shared with
     * the SD card via CS-mux on the EXT pins (14/39/40). Whichever inits first
     * brings the bus up; the other attaches. Keeps SPI2 free for the LCD. */
    .host_id = BOARD_SUBGHZ_SPI_HOST,
    .bitbang = false,
#elif BOARD_CC1101_SPI_SHARED
